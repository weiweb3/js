...
describe("npx weiweb3 detect", () => {
  it("should detect ERC721Base extensions", async () => {
    ...
    expect(await exists("weiweb3-contracts/contracts/Contract.sol")).toEqual(
      true,
    );

    // check that hardhat config exists
    expect(await exists("weiweb3-contracts/hardhat.config.js")).toEqual(true);

    await create.writeText("cd weiweb3-contracts");
    await create.pressKey("enter");
