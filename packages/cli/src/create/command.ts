...
export async function twCreate(pPath: string = "", options: any) {
  ...
  if (!projectPath) {
    const defaultName =
      projectType === "contract" ? "weiweb3-contracts" : "weiweb3-app";
    const res = await prompts({
      type: "text",
      name: "path",
  ...
  if (!projectPath) {
    console.log(
      "\nPlease specify the project directory:\n" +
        `  ${chalk.cyan("npx weiweb3 create")} ${chalk.green(
          "<project-directory>",
        )}\n` +
        "For example:\n" +
        `  ${chalk.cyan("npx weiweb3 create")} ${chalk.green(
        ...
        `Run ${chalk.cyan("npx weiweb3 --help")} to see all options.`,
    );
    process.exit(1);
  }
  ...
  if (options.template === true) {
    console.error(
      "Please provide an template name, otherwise remove the template option. Checkout some templates you can use here: https://github.com/weiweb3/",
    );
    process.exit(1);
  }
