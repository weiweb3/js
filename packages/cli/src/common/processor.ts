...
export async function processProject(
  ...
  if (projectType === "none") {
    if (command === "deploy") {
      info(
        "No contracts detected in this directory. Redirecting to the weiweb3 contract deployment page.",
      );
      return "https://weiweb3.com/contracts";
    }
    ...
    logger.info(``);
    logger.info(`     ${chalk.cyan(`npx weiweb3@latest create --contract`)}`);
    logger.info(``);
    process.exit(1);
  }
  ...
  const soliditySDKPackage = "@weiweb3/contracts";
