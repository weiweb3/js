 ...
 *
 * ```javascript
 * import { useDisconnect } from "@weiweb3/react"
 * ```
 *
 *
 * @example
...
export function useDisconnect(options?: { reconnectPrevious?: boolean }) {
  ...
  invariant(
    wagmiContext,
    `useDisconnect() can only be used inside <Weiweb3Provider />. If you are using <Weiweb3SDKProvider /> you will have to use your own connection logic.`,
  );
