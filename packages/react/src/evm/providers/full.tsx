...
import { Weiweb3ConfigProvider } from "../contexts/weiweb3-config";
import { Weiweb3SDKProvider, Weiweb3SDKProviderProps } from "./base";
...
/**
 * The possible props for the Weiweb3Provider.
 */
export interface Weiweb3ProviderProps<
  TSupportedChain extends SupportedChain = SupportedChain,
> {
  /**
   * The {@link SDKOptions | Weiweb3 SDK Options} to pass to the weiweb3 SDK
   ...
   * Defaults to just the name being passed as `weiweb3 powered dApp`.
   */
  ...
  /**
   * The configuration used for weiweb3 auth usage. Enables users to login
...
const defaultdAppMeta: DAppMetaData = {
  name: "weiweb3 powered dApp",
};

const defaultWalletConnectors: Required<
  Weiweb3ProviderProps["walletConnectors"]
> = ["metamask", "walletConnect", "walletLink"];

/**
 *
 * The `<Weiweb3Provider />` component lets you control what networks you want users to connect to, what types of wallets can connect to your app, and the settings for the [Typescript SDK](https://docs.weiweb3.com/typescript).
 *
 * @example
 * You can wrap your application with the provider as follows:
 *
 * ```jsx title="App.jsx"
 * import { Weiweb3Provider, ChainId } from "@weiweb3/react";
 *
 * const App = () => {
 *   return (
...
>({
  ...
  autoConnect = true,
  children,
}: React.PropsWithChildren<Weiweb3ProviderProps<TSupportedChain>>) => {
  ...
  const readonlySettings: SDKOptionsOutput["readonlySettings"] = useMemo(() => {
    ...
    try {
      ...
      console.error(
        `failed to configure rpc url for chain: "${desiredChainId}". Did you forget to pass "desiredChainId" to the <Weiweb3Provider /> component?`,
      );
  ...
  return (
    ...
    >
      <WagmiProvider {...wagmiProps}>
        <Weiweb3SDKProviderWagmiWrapper
          queryClient={queryClient}
...
const Weiweb3SDKProviderWagmiWrapper: React.FC<
  React.PropsWithChildren<Omit<Weiweb3SDKProviderProps, "signer" | "provider">>
> = ({ children, ...props }) => {
  const provider = useProvider();
  const [signer] = useSigner();
  return (
    <Weiweb3SDKProvider signer={signer.data} provider={provider} {...props}>
      {children}
    </Weiweb3SDKProvider>
  );
};
