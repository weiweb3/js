...
describe("Wallet Authentication", async () => {
  ...
  it("Should reject payload with incorrect domain", async () => {
    ...
    sdk.updateSignerOrProvider(adminWallet);
    try {
      sdk.auth.verify("test.weiweb3.com", payload);
      expect.fail();
    } catch (err) {
      expect(err.message).to.equal(
        "Expected domain 'test.weiweb3.com' does not match domain on payload 'weiweb3.com'",
      );
    }
  });
  ...
  it("Should reject token with incorrect domain", async () => {
    ...
    try {
      await sdk.auth.authenticate("test.weiweb3.com", token);
      expect.fail();
    } catch (err) {
      expect(err.message).to.contain(
        "Expected token to be for the domain 'test.weiweb3.com', but found token with domain 'weiweb3.com'",
      );
    }
  });
