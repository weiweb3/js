...
export const mochaHooks = {
  beforeAll: async () => {
    ...
    const weiweb3FactoryDeployer = (await new ethers.ContractFactory(
    ...
    const deployTxFactory = weiweb3FactoryDeployer.deployTransaction;
    await deployTxFactory.wait();
    const weiweb3RegistryAddress = await weiweb3FactoryDeployer.registry();
    registryAddress = weiweb3FactoryDeployer.address;

    await registryContract.grantRole(
      await registryContract.OPERATOR_ROLE(),
      weiweb3FactoryDeployer.address,
    );
    ...
    for (const contractType in CONTRACTS_MAP) {
      ...
      const tx = await weiweb3FactoryDeployer.addImplementation(
    ...
    process.env.registryAddress = weiweb3RegistryAddress;
    // eslint-disable-next-line turbo/no-undeclared-env-vars
    process.env.factoryAddress = weiweb3FactoryDeployer.address;
    // eslint-disable-next-line turbo/no-undeclared-env-vars
