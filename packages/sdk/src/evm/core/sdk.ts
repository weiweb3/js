...
import IWeiweb3ContractABI from "@weiweb3/contracts-js/dist/abis/IWeiweb3Contract.json";
import { Weiweb3Storage } from "@weiweb3/storage";
import { Contract, ContractInterface, ethers, Signer } from "ethers";
import invariant from "tiny-invariant";

/**
 * The main entry point for the weiweb3 SDK
 * @public
 */
export class Weiweb3SDK extends RPCConnectionHandler {
  /**
   * Get an instance of the weiweb3 SDK based on an existing ethers signer
   *
   * @example
   * ```javascript
   ...
   * const sdk = Weiweb3SDK.fromSigner(signer, "mainnet");
   * ```
   *
  ...
  static fromSigner(
    ...
    const sdk = new Weiweb3SDK(network || signer, options, storage);
    sdk.updateSignerOrProvider(signer);
    return sdk;
  }

  /**
   * Get an instance of the weiweb3 SDK based on a private key.
   *
   * @remarks
   ...
   * @example
   * ```javascript
   * const sdk = Weiweb3SDK.fromPrivateKey("SecretPrivateKey", "mainnet");
   * ```
   *
  ...
  static fromPrivateKey(
    ...
    return Weiweb3SDK.fromSigner(signer, network, options, storage);
  }
  ...
  /**
   * @internal
   * should never be accessed directly, use {@link Weiweb3SDK.getPublisher} instead
   */
  private _publisher: ContractPublisher;
  /**
   * Internal handler for uploading and downloading files
   */
  private storageHandler: Weiweb3Storage;
  /**
  ...
  /**
   * Get an instance of a Custom Weiweb3Contract
  ...
  public async getContractList(walletAddress: string) {
    ...
    return addressesWithContractTypes
      ...
      .map(({ address, contractType, metadata }) => {
        invariant(metadata, "All Weiweb3Contracts require metadata");
        return {
          address,
          contractType,
