...
import DeprecatedAbi from "@weiweb3/contracts-js/dist/abis/IDelayedRevealDeprecated.json";
import { TokensLazyMintedEvent } from "@weiweb3/contracts-js/dist/declarations/src/DropERC721";
...
> {
  ...
  private async isLegacyContract(): Promise<boolean> {
    if (
      hasFunction<IWeiweb3Contract>("contractVersion", this.contractWrapper)
    ) {
      try {
        const version =
